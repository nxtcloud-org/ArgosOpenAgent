...
호스트 OS 메트릭(CPU/MEM/Disk/Network) + 컨테이너 메트릭 + 시스템/컨테이너 로그를 중앙 모니터링 노드(Mimir + Loki)로 push합니다.
...
| 감사 | 명령 실행 기록 | cmdaudit (PROMPT_COMMAND) |
...
INSTALL_DOCKER=0 sudo ./install.sh 10.32.20.100

# 설치만 하고 기동은 안 함
COMPOSE_UP=0 sudo ./install.sh 10.32.20.100

# 컨테이너 로그 필터 (매칭되는 줄만 전송)
LOG_KEEP_REGEX="(?i)(error|warn|fatal)" sudo ./install.sh 10.32.20.100

# 설치 경로 변경
ARGOS_BASE_DIR=/home/ec2-user/agent sudo ./install.sh 10.32.20.100
```
...
```
/opt/argos-agent/
├── docker-compose.yml    # Alloy + autoheal 컨테이너
